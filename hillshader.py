...
################################################################################
def hillshader(ras, output_folder, z_factor):
    """ creates hillshades for all Tifs in the list"""
    # Create a hillshade of the current raster
    outpath = os.path.join(output_folder, "hill_" + ras[:-4] + ".tif")
    version_count = 0
    ...
    while os.path.exists(outpath) == True:
        ...
        arcpy.HillShade_3d(ras, outpath, "#", "#", "SHADOWS", z_factor)
...
def main(rast_folder, output_folder,z_factor):
    ...
    for ras in rast_list:
        hillshader(ras, output_folder,z_factor)
...
if __name__ == '__main__':
    ...
    z_factor = arcpy.GetParameterAsText(2)


    main(rast_folder, output_folder, z_factor)